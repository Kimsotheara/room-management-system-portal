  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ filteredItems.length }} of {{ store.items.length }} users
        </p>
      </div>
      <button
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div v-else-if="!pagedItems.length" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ keyword || filterStatus ? 'Try adjusting your search or filters' : 'Create the first user to get started' }}
        </p>
      </div>
    ...
    <UsersAssignRoleModal v-model="showAssignRole" :user="selectedUser" @updated="store.fetchAll()" />
    <UiConfirmDialog
      v-model="showDelete"
      title="Delete User"
...
const filteredItems = computed(() => {
  ...
  if (filterStatus.value !== '') {
    ...
    items = items.filter(u => u.isActive === active)
  }
  return items
})
